  ...
  <script>
        $(document).ready(function(){
            $("#perpage").change(function (){
                var url = "<?php echo site_url('users/index');?>/"+$(this).val()+"<?=$get_suffix?>";
                $('.content').load(url);
                return false;
            })
...
<!-- Batas Header -->
        ...
        <div class="row" style="margin-bottom: 10px">
            ...
            <?php
                ...
                if(in_array($_idmenu->id_sub, explode(',', $input_data)) or $level_user=='administrator'){?>
                  <a href="#" onclick="opened_2('users/create', '<?=$_GET['p']?>');" class="btn btn-primary">Create</a>
                <?php }
            ?>
	    </div>
        </div>
        <!-- Perpage -->
            ...
            <div class="col-lg-4">
                <?=form_open(site_url('users/index/'), array('method'=>'get', 'class'=>'jsform'))?>
...
//            $start = 0;
		    ...
		    <td style="text-align:center">
                <div class="btn-group">
                    <a href="#" onclick="opened_2('users/read/<?=$users->id?>', '<?=$_GET['p']?>');" class="btn btn-default btn-sm" title="View"><i class="fa fa-eye"></i></a>
                    <?php 
                    if(in_array($_idmenu->id_sub, explode(',', $edit_data)) or $level_user=='administrator'){ ?>
                        <a href="#" onclick="opened_2('users/access/<?=$users->id?>', '<?=$_GET['p']?>');" class="btn btn-success btn-sm" title="Access"><i class="fa fa-expeditedssl"></i></a>
                        <a href="#" onclick="opened_2('users/update/<?=$users->id?>', '<?=$_GET['p']?>');" class="btn btn-primary btn-sm" title="Edit"><i class="fa fa-pencil-square-o"></i></a>
                    <?php }
                    if(in_array($_idmenu->id_sub, explode(',', $delete_data)) or $level_user=='administrator'){ ?>
                        <a href="#" onclick="if(confirm('Are you sure?')) opened_2('users/delete/<?=$users->id?>', '<?=$_GET['p']?>');" class="btn btn-danger btn-sm" title="Delete"><i class="fa fa-trash-o"></i></a>
                    <?php }
                    ?>
                </div>
		    </td>
